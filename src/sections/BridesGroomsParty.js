  ...
  return (
    <Box sx={{ textAlign: 'center' }}>
      ...
      <Typography variant="h6" sx={{
        fontSize: '0.95rem', color: '#2c1f14',
        fontFamily: "'Cormorant Garamond', serif",
        fontStyle: 'italic', mb: 0.5,
      }}>
        {person.name}
      </Typography>
      <Typography sx={{ color: '#c9a96e', fontSize: '0.65rem', letterSpacing: '0.15em' }}>
        {person.role}
      </Typography>
    </Box>
  );
...
export default function BridesGroomsParty() {
  return (
    <Box id="turma" sx={{ py: { xs: 8, md: 14 }, background: '#fff' }}>
      <Box sx={{ maxWidth: 1100, mx: 'auto', px: { xs: 2, md: 4 } }}>
        ...
        <Box sx={{ mb: 10 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 6, gap: 3 }}>
            ...
            <Typography variant="h4" sx={{ fontSize: { xs: '1.5rem', md: '2rem' }, color: '#2c1f14', fontStyle: 'italic', whiteSpace: 'nowrap' }}>
              Damas de Honra
            </Typography>
