      ...
      <Box sx={{ position: 'relative', zIndex: 1, maxWidth: 1200, mx: 'auto', px: { xs: 2, md: 4 } }}>
            ...
            return (
              <Grid item xs={12} sm={6} md={3} key={i} >
                <Box sx={{
                  ...
                  <Typography variant="h5" sx={{ fontSize: '1.1rem', color: colors.darkBrown, mb: 2, lineHeight: 1.3 }}>
                    {card.title}
                  </Typography>
                  {card.details.map((d, j) => (
                    <Typography key={j} sx={{ color: colors.mediumBrown, fontSize: '0.85rem', fontWeight: 300, lineHeight: 1.8 }}>
                      {d}
                    </Typography>
                  ))}
                  {card.map && (
        ...
        <Box sx={{
          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <Typography variant="h3" sx={{ color: '#fff', fontSize: { xs: '1.8rem', md: '2.5rem' }, fontStyle: 'italic', mb: 2 }}>
              Esporte Fino
            </Typography>
            <Typography sx={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9rem', fontWeight: 300, maxWidth: 500, mx: 'auto' }}>
